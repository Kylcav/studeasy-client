...
export default function ProfileTeacher() {
  ...
  // ✅ Charger la photo via backend (stream)
  useEffect(() => {
    let mounted = true;
  ...
  const displayName = useMemo(() => {
    return user?.name ?? user?.fullName ?? "Teacher";
  }, [user]);
  ...
  const onUpload = async () => {
    ...
    try {
      ...
      // upload -> backend renvoie user (avec profileImage)
      const updated = await uploadProfileImage(userId, file);

      // ✅ met à jour le user global (sidebar etc.)
      setUser(updated);

      // ✅ recharge le binaire via API
